...
State g_state = IDLE;
Release g_release;
std::string g_error, g_reason, g_bundle;
...
// Worker: reads the saved file back from the SD card and compares it with what GitHub published.
std::string verify(const std::string& path, const Release& rel, const Job& job) {
    FILE* f = fopen(path.c_str(), "rb");
    if (!f) return "Couldn't read the download back";
    sha256::Hasher h;
    std::vector<char> buf(256 * 1024);
    int64_t total = 0;
    char magic[4] = {};
    size_t n;
    while (!job.cancel && (n = fread(buf.data(), 1, buf.size(), f)) > 0) {
        if (total == 0 && n >= 4) memcpy(magic, buf.data(), 4);
        h.update(buf.data(), n);
        total += n;
    }
    ...
    if (failed || total != rel.size) return "Couldn't read the download back";
    if (memcmp(magic, "WUHB", 4)) return "The download isn't a Wii U app";
    if (h.hex() != rel.sha256) return "The download didn't match GitHub's checksum";
    return "";
}
...
std::string fetch(const Release& rel, const std::string& dest, Job& job) {
    ...
    std::string err = job.cancel                         ? "Cancelled"
                      ...
                      : too_big                          ? "The download was bigger than GitHub said"
                      : r.status >= 300                  ? util::fmt("GitHub answered %ld", r.status)
                      : !r.error.empty()                 ? r.error
...
void on_checked(CheckResult r, bool automatic_check) {
    ...
    if (!newer(r.rel.version, APP_VERSION)) {
    ...
    g_release = r.rel;
    g_state = AVAILABLE;
    log_message(LOG_OK, "Update", "CoffeeFlix %s is out (this is %s)", r.rel.version.c_str(), APP_VERSION);
    if (automatic_check && !automatic() && store::get_str("update_notified") != r.rel.version) {
        store::set_str("update_notified", r.rel.version);
        ui::toast(util::fmt("CoffeeFlix %s is available. Update it in Settings.", r.rel.version.c_str()), ic::CLOUD_DOWNLOAD);
    }
}
...
void start_check(bool automatic_check) {
    g_state = CHECKING;
    g_error.clear();
    g_scope.run<CheckResult>([] { return fetch_latest(); },
                             [automatic_check](CheckResult r) { on_checked(std::move(r), automatic_check); });
}
...
void swap_back() {
    ...
    // Going back from a release: automatic updates leave that one alone.
    store::set_str("update_skip", newer(APP_VERSION, prev) ? APP_VERSION : "");
    store::save_now();
...
void init() {
    ...
    if (have && newer(ready.version, APP_VERSION) && is_sha256(ready.sha256) && file_size(dl) == ready.size) {
        reverify(ready);
...
void tick() {
    ...
    // Once a day, a while after start-up so it doesn't compete with the first screens.
    bool settled = g_state == IDLE || g_state == UP_TO_DATE || g_state == FAILED;
    if (settled && now - g_started > 20 && now >= g_next_check && util::unix_time() - g_last_checked >= DAY &&
        platform::network_connected()) {
        g_next_check = now + 3600;
        start_check(true);
    }
    if (g_state == AVAILABLE && automatic() && !g_declined && now >= g_next_download &&
        store::get_str("update_skip") != g_release.version && !player_busy()) {
...
void download() {
    ...
    g_scope.run<std::string>([rel, dest, job] { return fetch(rel, dest, *job); }, [rel, job](std::string err) {
        ...
        remember_ready(&rel);
        g_state = READY;
        log_message(LOG_OK, "Update", "CoffeeFlix %s is downloaded and matches GitHub's checksum", rel.version.c_str());
        ui::toast(util::fmt("CoffeeFlix %s is ready. It installs when you close CoffeeFlix.", rel.version.c_str()),
                  ic::CHECK_CIRCLE);
    });
}
