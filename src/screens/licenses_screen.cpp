...
const Document DOCUMENTS[] = {
    {"NOTICES.txt", "Notices", "Everything CoffeeFlix includes, with credits and license texts"},
    {"PolyForm-Noncommercial-1.0.0.txt", "CoffeeFlix license", "PolyForm Noncommercial 1.0.0"},
    {"LGPL-2.1.txt", "GNU LGPL 2.1", "libsmb2"},
    {"LGPL-3.0.txt", "GNU LGPL 3.0", "FFmpeg"},
    {"GPL-3.0.txt", "GNU GPL 3.0", "The terms LGPL 3.0 builds on"},
    {"Apache-2.0.txt", "Apache License 2.0", "Mbed TLS, Material Icons"},
    {"MPL-2.0.txt", "Mozilla Public License 2.0", "Certificate authority list"},
    {"FreeType-FTL.txt", "FreeType License", "FreeType"},
    {"Inter-OFL.txt", "SIL Open Font License 1.1", "Inter font"},
    {"MaterialIcons.txt", "Material Icons", "Icon font details"},
};
...
public:
    TextScreen(std::string title, const std::string& path) : title_(std::move(title)) {
        std::string text;
        if (!util::read_file(path, text)) text = "This file is missing from this copy of CoffeeFlix.";
...
public:
    ...
    void frame() override {
        ...
        float y = page_.y(48);
        text::draw(font::headline, x0, y, "Licenses", t.text);
        text::draw_wrapped(font::body, Rect(x0, y + 50, w, 60),
                           "CoffeeFlix is source-available for noncommercial use. It includes open-source libraries "
        ...
        for (size_t i = 0; i < sizeof(DOCUMENTS) / sizeof(DOCUMENTS[0]); i++) {
            ...
            if (value_row(iid, Rect(x0, y, w, 64), d.title, d.subtitle, ic::DESCRIPTION, g, i == 0 ? F_DEFAULT : 0))
                app::push(std::make_unique<TextScreen>(d.title, platform::content_dir() + "/licenses/" + d.file));
